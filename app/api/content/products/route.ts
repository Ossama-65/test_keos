...
import { getSiteContent, updateSection } from '@/lib/mongodb';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!content) {
      return NextResponse.json({ error: 'Contenu non initialisé' }, { status: 500 });
    }
    
    const products = content.products || [];
    
    // Générer un ID unique
    const maxId = products.reduce((max: number, p: { id: string }) => {
    ...
    await updateSection('products', products);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (content.bestsellers?.products) {
      ...
      await updateSection('bestsellers', content.bestsellers);
    }
